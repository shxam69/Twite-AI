...
export default function Sidebar({ isOpen, onClose }) {
  ...
  // Base nav items visible to all authenticated roles
  const allNavItems = [
    {
      to: '/dashboard',
      label: 'Dashboard',
    ...
    },
    {
      to: '/employees',
      label: 'Employees',
      roles: ['admin'],  // Admin only — employees must not see this
      icon: (
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  // Filter nav items to only those the current user's role is allowed to see
          ...
          {navItems.map((item) =>
            item.isExternal ? (
              <a
                ...
                className="flex items-center space-x-3 px-3.5 py-2.5 rounded-lg text-sm font-medium text-slate-600 hover:text-slate-900 hover:bg-slate-100 transition-all cursor-pointer"
              >
                {item.icon}
