...
export default function AdminHelpRequestsPage() {
  ...
  const getTypeBadge = (type) => {
    switch (type) {
      case 'QR_SCAN':
        return <span className="px-2.5 py-0.5 rounded-md text-xs font-semibold bg-purple-100 text-purple-800 border border-purple-200">QR Scan</span>;
      case 'LOCATION_FAILED':
        return <span className="px-2.5 py-0.5 rounded-md text-xs font-semibold bg-rose-100 text-rose-800 border border-rose-200">GPS / Geofence</span>;
      case 'CHECKIN_FAILED':
        return <span className="px-2.5 py-0.5 rounded-md text-xs font-semibold bg-amber-100 text-amber-800 border border-amber-200">Check-in Error</span>;
      case 'CHECKOUT_FAILED':
        return <span className="px-2.5 py-0.5 rounded-md text-xs font-semibold bg-indigo-100 text-indigo-800 border border-indigo-200">Check-out Error</span>;
      default:
        return <span className="px-2.5 py-0.5 rounded-md text-xs font-semibold bg-slate-100 text-slate-700 border border-slate-200">Other Support</span>;
    }
  };
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-xs">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800 tracking-tight">Employee Help Requests</h1>
            <p className="text-xs text-slate-500">Manage and resolve employee support issues</p>
          </div>
        </div>
              ...
              {loading ? (
                [...Array(4)].map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    ...
                    <td className="py-4 px-5 text-right"><div className="h-6 bg-slate-200 rounded w-16 ml-auto" /></td>
                  </tr>
                ))
              ) : requests.length > 0 ? (
                requests.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-4 px-5 max-w-xs text-slate-700 text-xs">
                      {item.message || <span className="text-slate-400 italic">No details provided</span>}
                    </td>
                      ...
                      {item.status === 'OPEN' ? (
                        ...
                        <div className="space-y-0.5">
                          ...
                          {item.resolved_by_name && (
                            <span className="text-[10px] text-slate-400 block">by {item.resolved_by_name}</span>
                          )}
                        </div>
                      )}
                    </td>
